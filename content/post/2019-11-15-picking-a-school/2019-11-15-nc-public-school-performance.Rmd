...
metrics and how I use them to make what is hopefully an informed decision. Of
course, the choice of school depends on more than test scores, but they are a
...
kids got a lot of questions right and an "F" means they didn't.

The growth score is new to me, but given how prominent the color of the school
is compared to it's letter grade, it seems important.
...
fourth grade level. Looking at student growth reveals that Teacher A is actually
doing a better job.
...
status or previous competency level.
...
Unfortunately, I was unable to find more than vague references to how the EVAAS
scores were calculated. I can't find a statistical model or published
...
Another thing I learned was that the EOG and EVAAS don't apply to
private schools, which complicates comparisons. The private schools I contacted
all use the [Iowa Test of Basic
Skills](https://en.wikipedia.org/wiki/Iowa_Assessments) (ITBS). Some were more
forthcoming with their scores than others, and a systematic comparison of
private schools was difficult. I also discovered the National Assessment of
Educational Progress ([NAEP](https://nces.ed.gov/nationsreportcard/)), which is
administered to both private and public schools in fourth, eighth and twelfth
grades, but viewing data by school requires a license.

## School demographics

The test scores in the scatter plot above are for the schools as a whole.
Several studies, including one using the NAEP data, suggest that achievement
differences between schools are due more to demographic differences than
anything else
([link](https://nces.ed.gov/nationsreportcard/pubs/studies/2006461.asp)). This
may justify the weak relationship between EVAAS and EOG scores. It definitely
means that school composition is critical. There are several online sources that
show the demographics of individual schools
([example](https://www.greatschools.org/)), but I found it helpful to map the
...
aggregate school metrics. What is needed is to compare students from similar
...
The chart below breaks down the EOG scores by school and student subgroup. In
short, every subgroup scored better at Peak Charter except for white students.
Further, the difference in white students is small compared to how well they
perform at all three schools.
...
concerning, but there could be several explanations (e.g. new students
...
copy of their letter to parents from last year. Their students did very well,
with most classes scoring at 90% or above.

## Comparing public and private

The easy decision is to select Peak Charter Academy as the preferred public
option. Of course, admission to Peak is done by lottery, so it isn't
guaranteed.

It is harder to judge if the private schools are worth the money. It is
impossible to compare private to public directly given the different tests
used.

While researching the history of testing in NC, I realized the ITBS was used by
North Carolina until the 2001-2002 school year. While private schools are still
required to do yearly testing, they stopped for public schools.
...
statewide scores. Based on my research, I expect the scores from Wake County to
be higher than the state average and the scores for Apex schools to be higher
than the average for Wake County. Still, guessing where they would score today
is impossible.

The private schools certainly had high ITBS scores last year, but their
attendance is heavily skewed toward Asian and White students. These are the
highest-performing demographic across the county. It is hard to say how much of
the ITBS scores are due to simple demographics.
