...
The chart below shows the EOG scores by school and race. Black and hispanic
students get the best scores at Peak while white students score higher at Penny
Road.
...
race_chart <- chart_tbl %>%
  filter(
    subgroup %in% c("White", "Black", "Hispanic"),
    school %in% local_public_schools
  ) %>% mutate(
    # x_label = map(function(x) word(x)[1], school)
    x_label = map_chr(school, function(x) word(x)[1])
  )

ggplot(data = race_chart) +
  # geom_point(aes(x = pct_4, y = growth_index, color = subgroup)) +
  geom_bar(aes(x = x_label, y = pct_4, fill = subgroup), stat = "identity") +
  # geom_text(
  #   aes(x = pct_4, y = growth_index, label = school),
  #   check_overlap = TRUE, size = 3, nudge_y = .2
  # ) +
  facet_wrap(vars(subgroup)) +
  ggtitle("EOG Scores by Race") +
  xlab("School") + 
  ylab("EOG CCR") +
  guides(fill = FALSE)
```
